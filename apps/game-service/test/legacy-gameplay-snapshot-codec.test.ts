...
describe("domain gameplay compatibility snapshot decoder", () => {
  ...
  it.each([
    ...
    const engine = new GameEngine({
      enableSecondBidding: scenario.enableSecondBidding,
      humanCount: 4,
    ...
    const openingBidding = structuredClone(engine.getSnapshot().bidding);
    const source: LegacyGameplaySnapshotRecord = {
    ...
    const applied = applyGameplayCommand(before, command);
    if (!applied.ok) throw new Error("Expected legal indicator selection");
    ...
    expect(legacy.trump.card?.cardId).toBe(indicator.id);
    expect(legacy.bidding.actions).toEqual(openingBidding.actions);
    expect(legacy.bidding.actedInRound).toEqual(openingBidding.actedInRound);
    expect(legacy.bidding.noBidPasses).toBe(openingBidding.noBidPasses);
    expect(legacy.bidding.order).toEqual(openingBidding.order);
    expect(legacy.bidding.passesAfterBid).toBe(openingBidding.passesAfterBid);
    expect(legacy.seats.map((seat) => seat.hand.length)).toEqual(
      applied.hand.deal.hands.map((cards) => cards.length),
    );
  });
