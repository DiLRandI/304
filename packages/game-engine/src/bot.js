import { compareRank } from "./cardData.js";
...
function chooseBidFromHand(state, seat, legalActions, phase) {
  ...
  if (phase === "second") {
    const options = legalBids(
      legalActions,
      state.profile.minEightCardBid,
      state.profile.eightCardBidStep,
    );
    if (
      !options.length ||
      (state.profileId === "classic_304_4p" && countPoints(hand) < 100)
    ) {
      return null;
    }
    for (const option of options.reverse()) {
      if (
        option >= 250 &&
        option <= state.bidding.currentBid + 20 + Math.floor(handScore / 4)
      ) {
        return option;
      }
    }
...
function chooseTrumpSeatChoice(state, seat) {
  ...
  for (const [suit] of Object.entries(bySuit)) {
    ...
    const score =
      ...
      cards.reduce(
        (sum, card) =>
          sum + (card.rank === "J" ? 50 : card.rank === "9" ? 30 : 0),
        0,
      );
...
function chooseTrumpOpen(state, legalActions) {
...
function choosePlay(state, seat, legalActions) {
  ...
  const hand = state.seats[seat].hand;
  const follows = legalActions.filter(
    (a) => a.type === "PLAY_CARD" && !a.faceDown,
  );
  const anyFaceDown = legalActions.filter(
    (a) => a.type === "PLAY_CARD" && a.faceDown,
  );
  ...
  if (follows.length === 0) {
    ...
    if (
      anyFaceDown.length > 0 &&
      state.trumpCard &&
      state.trumpCard.suit === state.trumpSuit
    ) {
      return (
        anyFaceDown.find((action) => action.card.rank === "J") ||
        anyFaceDown[anyFaceDown.length - 1]
      );
    }
  }
  ...
  if (state.trumpClosed && anyFaceDown.length > 0 && state.trumpCard) {
    const trumpCards = follows.filter(
      (action) => action.card.suit === state.trumpSuit,
    );
  ...
  if (
    state.leadingSeat === seat &&
    hand.length > 1 &&
    state.trumpCard &&
    state.trumpCard.suit === state.currentLedSuit
  ) {
...
export function pickBotAction(state, seatIndex) {
  ...
  if (phase === "trump_selection") {
    ...
    if (selected) {
      const exact = legalActions.find(
        (a) => a.type === "SELECT_TRUMP" && a.cardId === selected,
      );
  ...
  if (phase === "trump_choice") {
    const choice = chooseTrumpOpen(state, legalActions);
    if (difficulty === "easy") {
