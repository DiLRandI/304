...
test("Gameplay recovery persistence depends on a Gameplay-owned store port", async () => {
  const recoverySource = await readFile(
    path.join(
      repoRoot,
      "apps/game-service/src/contexts/gameplay/adapters/persistence/domain-gameplay-recovery.ts",
...
test("schema-v1 result acknowledgement tests use frozen compatibility records", async () => {
  const acknowledgementTestSource = await readFile(
    path.join(
      repoRoot,
      "apps/game-service/test/legacy-gameplay-result-acknowledgement.test.ts",
    ),
    "utf8",
  );

  assert.match(acknowledgementTestSource, /legacy-gameplay-snapshot-fixture/);
  assert.doesNotMatch(
    acknowledgementTestSource,
    /@three-zero-four\/game-engine/,
  );
});
...
test("schema-v1 trump choice tests use frozen compatibility records", async () => {
  const choiceTestSource = await readFile(
    path.join(
      repoRoot,
      "apps/game-service/test/legacy-gameplay-trump-choice.test.ts",
    ),
    "utf8",
  );

  assert.match(choiceTestSource, /legacy-gameplay-snapshot-fixture/);
